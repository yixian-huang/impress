...
SMOKE_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/inkless-two-instances.XXXXXX")"
PORT_A="${INKLESS_SMOKE_PORT_A:-18088}"
PORT_B="${INKLESS_SMOKE_PORT_B:-18089}"
BINARY="${INKLESS_SMOKE_BINARY:-${SMOKE_ROOT}/inkless-server}"
PID_A=""
PID_B=""
...
cleanup() {
  stop_pid "${PID_A}"
  stop_pid "${PID_B}"
  if [[ "${INKLESS_SMOKE_KEEP:-false}" == "true" ]]; then
...
for port in "${PORT_A}" "${PORT_B}"; do
  if curl -fsS --max-time 1 "http://127.0.0.1:${port}/health" >/dev/null 2>&1; then
    fail "port ${port} is already serving HTTP; set INKLESS_SMOKE_PORT_A/B"
  fi
done
[[ "${PORT_A}" != "${PORT_B}" ]] || fail "instance ports must differ"

if [[ -z "${INKLESS_SMOKE_BINARY:-}" ]]; then
...
for instance in a b; do
  ...
  cp "${BINARY}" "${root}/backend/versions/v1/inkless-server"
  cp "${BINARY}" "${root}/backend/versions/v2/inkless-server"
...
start_instance() {
  ...
  local instance_binary="${root}/backend/current/inkless-server"
  PORT="${port}" \
  ENV=production \
  SEED_MODE=demo \
  DB_DSN="file:${root}/data/inkless.db?cache=shared&mode=rwc" \
...
stop_pid "${PID_A}"
PID_A=""
rm -f "${SMOKE_ROOT}/a/data/inkless.db-wal" "${SMOKE_ROOT}/a/data/inkless.db-shm"
gzip -dc "${BACKUP_A}" >"${SMOKE_ROOT}/a/data/inkless.db"
...
[[ -s "${SMOKE_ROOT}/a/data/inkless.db" && -s "${SMOKE_ROOT}/b/data/inkless.db" ]] || fail "isolated databases missing"
