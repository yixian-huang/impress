...
export function VersionHistoryList({
  ...
  return (
    <>
      <div className="px-4 py-3 border-b border-slate-100 space-y-2 flex-shrink-0 bg-slate-50">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {loading ? (
          ...
          <div className="text-center text-slate-500 py-8 text-sm">
            暂无版本记录
            <p className="mt-1 text-xs text-slate-400">保存或发布文章后会自动生成版本快照。</p>
          </div>
        ) : (
          <div className="space-y-2">
            {versions.map((v) => (
              <div
                key={v.id}
                className="p-3 border border-slate-200 rounded-lg hover:border-blue-200 transition-colors"
              >
                ...
                <div className="text-xs text-slate-500 mt-1">{formatVersionTime(v.createdAt)}</div>
                {(v.zhTitle || v.summary) && (
