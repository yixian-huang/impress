...
export function DiffView({ lines }: { lines: DiffLine[] }) {
  if (lines.length === 0) {
    return <div className="text-sm text-slate-400 py-8 text-center">无差异</div>;
  }
  const changed = lines.filter((l) => l.op !== "equal").length;
  return (
    <div className="font-mono text-xs leading-5 border border-slate-200 rounded-lg overflow-hidden">
      <div className="px-3 py-1.5 bg-slate-50 border-b border-slate-200 text-slate-500 flex justify-between">
        <span>行级差异</span>
        ...
        {lines.map((line, idx) => {
          ...
          return (
            <div key={idx} className={`flex ${bg} border-b border-gray-50 last:border-0`}>
              <span className="w-10 flex-shrink-0 text-right pr-2 text-slate-400 select-none tabular-nums">
                {line.leftLine ?? ""}
              </span>
              <span className="w-10 flex-shrink-0 text-right pr-2 text-slate-400 select-none tabular-nums">
                {line.rightLine ?? ""}
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        {same ? (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-500">相同</span>
        ) : (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-100 text-amber-800">有变更</span>
        )}
      </div>
      {same ? (
        <div className="text-xs text-slate-500 bg-slate-50 rounded p-2 max-h-24 overflow-auto whitespace-pre-wrap">
          {leftText || <span className="italic text-slate-400">（空）</span>}
        </div>
      ) : (
